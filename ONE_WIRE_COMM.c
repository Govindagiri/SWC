...
Step1 :- Initialize the timer and add ONE_WIRE_ISR(); inside TIMx_IRQHandler().
...
Step3 :- Set Timer to Input Capture Mode with both-edge polarity. For minimum frequency measurement, 
         e.g., 20 Hz: With TIMER_CLOCK = 64 MHz, 
            ...
            minimum readable frequency = 64 MHz / 64 = 1 MHz, 
                                       = 1 MHz / 65535
                                       =  15.2 Hz. 
        Note: input Frequency below 15.2 hz may result inaccurate ON/OFF time .
...
//#endif
    ...
    if (count == 0) 
    {
      if(State_val == 1)
      {
       IC_Val1 = Get_IC_Value(TIMx,Channel);
       count++;
       //first_high = 1;
      }
      else 
    ...
    /* Check Second EDGE */
    else if ((State_val != first_high) && count == 1) 
    {
      IC_Val2 = Get_IC_Value(TIMx,Channel);
      count++;
    }
    
    /* Check Third EDGE */
    else if ((State_val == first_high) && count == 2) 
    {
      IC_Val3 = Get_IC_Value(TIMx,Channel);
      count = 1;
...
void ProcessDataBits(void) 
{
 ...
 /* Checking High Bit */
  if (Off_time < 50 && On_time > 50) 
  {
    current_byte = (current_byte << 1) | 0x01;
  } 
  
  /* Check Low Bit */
  else if (Off_time > 50 && On_time < 50) 
  {
    current_byte <<= 1;
  }
  ...
  if (bit_index == 8) 
  {
    ...
    if (data_index > 11)
    {
      ...
      if (!checksum) 
      {
          for( uint8_t i =0; i < sizeof(ONE_WIRE.data); i++)
          {
            ONE_WIRE.Reg[i]=ONE_WIRE.data[i];
          }
          if(ONE_WIRE.Reg[0] == 255)
            first_high = 0;
      } 
      else 
      {
...
// 
//         if((frequency<65)&&(Flag_startbit==RESET))                              
